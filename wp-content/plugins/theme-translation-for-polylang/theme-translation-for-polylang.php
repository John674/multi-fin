...
class Polylang_Theme_Translation {
    ...
    const SETTINGS_FORCE_TRANSLATE_ADMIN_DASHBOARD = 'custom_pll_settings_force_translate_admin_dashboard';
...
function process_polylang_theme_translation_wp_loaded() {
    ...
    if (current_user_can('manage_options') && $pagenow === 'admin.php') {
        ...
        if (isset($_POST['action_settings'])) {
            ...
            // others settings:
            $translateAdmin = $_POST['force_translate_admin'] ?? NULL;
            if (!add_option(Polylang_Theme_Translation::SETTINGS_FORCE_TRANSLATE_ADMIN_DASHBOARD, $translateAdmin)) {
                update_option(Polylang_Theme_Translation::SETTINGS_FORCE_TRANSLATE_ADMIN_DASHBOARD, $translateAdmin);
            }
...
function custom_pll_settings_active_tab_import_export_strings() {
    ...
    $data = [
        'settings' => $settings,
        'force_translate_admin' => get_option(Polylang_Theme_Translation::SETTINGS_FORCE_TRANSLATE_ADMIN_DASHBOARD),
...
function pll_admin_current_language_tt_for_polylang($curlang, $admin) {
    if (is_admin()) {
        $translateAdmin = get_option(Polylang_Theme_Translation::SETTINGS_FORCE_TRANSLATE_ADMIN_DASHBOARD);
        if (!empty($translateAdmin)) {
            $lang = pll_default_language();
            $curlang = PLL()->model->get_language($lang);
        }
        else {
            $curlang = NULL;
        }
